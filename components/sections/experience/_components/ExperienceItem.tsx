...
export const ExperienceItem = ({ item, index }: ExperienceItemProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative w-full flex flex-col gap-6 rounded-2xl sm:rounded-3xl border border-card-border/60 bg-muted/30 p-6 sm:p-8 backdrop-blur-md transition-all duration-300 hover:border-border hover:bg-muted/50 hover:shadow-xl",
      )}
    >
      {/* Header Section: Company, Role & Date */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-card-border/60 pb-4">
        <div className="flex flex-col gap-1">
          <a
            href={item.companySite}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 font-heading text-xl font-bold uppercase tracking-tight text-primary transition-colors hover:text-accent sm:text-2xl"
          >
        ...
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted/90 border border-card-border text-muted-foreground font-mono text-xs font-medium shrink-0 self-start sm:self-auto">
      ...
      <div className="flex flex-col justify-between gap-6">
        <ul className="flex flex-col gap-2.5 font-mono text-xs sm:text-sm leading-relaxed text-muted-foreground">
